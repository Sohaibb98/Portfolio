...
void DisplayFunction() {
	...
	while (X < 10)
	{
		...
		{
			if (arrcookie[X][Y] == 0)
				DrawCookie(CK_BURGER, ypixel, xpixel, cwidth, cheight);
			else if (arrcookie[X][Y] == 1)
				DrawCookie(CK_BISCUIT, ypixel, xpixel, cwidth, cheight);
			else if (arrcookie[X][Y] == 2)
				DrawCookie(CK_STAR, ypixel, xpixel, cwidth, cheight);
			else if (arrcookie[X][Y] == 3)
				DrawCookie(CK_CUPCAKE, ypixel, xpixel, cwidth, cheight);
			else if (arrcookie[X][Y] == 4)
				DrawCookie(CK_DONUT, ypixel, xpixel, cwidth, cheight);
			else if (arrcookie[X][Y] == 5)
				DrawCookie(CK_CROISSONT, ypixel, xpixel, cwidth, cheight);
			Y = Y + 1;
			ypixel = ypixel + 60;
		}
		X = X + 1;
	...
	while (X < 10)
	{
		...
		while (Y < 15)
		{
			//this if statment checks if the cookies are crushable and number of columns does not exceed 13 and 
			//number of rows doesn't exceed 8 or else the index will go out of range.
			if ((arrcookie[X][Y] == arrcookie[X][Y + 1]) && (arrcookie[X][Y]== arrcookie[X][Y + 2] && Y<13))
			{
				tempcount = 3;
				while (arrcookie[X][Y + tempcount] == arrcookie[X][Y])
				{
					arrcookie[X][Y + tempcount] = -1;
			...
			if ((arrcookie[X][Y] == arrcookie[X + 1][Y]) && (arrcookie[X][Y] == arrcookie[X + 2][Y] && X<8))
			{
				tempcount = 3;
				while (arrcookie[X + tempcount][Y] == arrcookie[X][Y])
				{
					arrcookie[X + tempcount][Y] = -1;
					tempcount = tempcount + 1;
				}
				arrcookie[X][Y] = -1;
				arrcookie[X + 1][Y] = -1;
	...
	while (X < 10)
	{
		...
		while (Y < 15)
		{
			if (arrcookie[X-1][Y] == -1)
			{
				temp = arrcookie[X - 1][Y];
				arrcookie[X - 1][Y]= arrcookie[X][Y];
				arrcookie[X][Y] = temp;
	...
	//Generating random cookies on the top row until the canvas is refilled
	Y = 0;
	while (Y < 15)
	{
		if (arrcookie[9][Y] == -1)
		{
			arrcookie[9][Y] = GetRandInRange(0, 5);
			if (mc == 1)
			{
				score = score + 1;
				mc = 0;
				//mc is abbreviated for "mouse clicked" and is there to add the score only when the user crushes the cookies and it was not auto-crushed.
			}
		}
		Y = Y + 1;
	}
...
void MouseClicked(int button, int state, int x, int y) {
	...
	if (button == GLUT_LEFT_BUTTON) // dealing only with left button
	{
		...
		//the following set of if statements check whether the swapped cookies are 1 position either above, below, left or right to each other.
		if (A == 0)
		{
			if (l == j  && (k == i + 1 || k == i - 1))
			{
...
int main(int argc, char*argv[]) {
	...
	while (X < 10)
	{
		...
		while (Y < 15)
		{
			arrcookie[X][Y] = GetRandInRange(0, 5);
			Y = Y + 1;
		}
		X = X + 1;
	}
